...
class Gemini_LLM:
    ...
    def inference(self, prompt: str, generation_config=None) -> str:
        ...
        if (
            ...
            raise ValueError(
                "Invalid operation: The response contains no valid candidates or content parts."
            )
...
class PubMedNeuralRetriever:
    def __init__(
        ...
        if dense_search:
            ...
            embeddings_paths, pmids_paths = generate_paths(
                base_dir, init_chunk=10, end_chunks=36
            )
        ...
        if self.dense_search:
            ...
            for _ in range(num_query_attempts):
                pubmed_query = self.generate_pubmed_query(
                    question, failure_cases=failure_cases
                )
    ...
    def fetch_article_data(self, article_ids: List[str]):
        ...
        article_data = Entrez.read(articles)
        return article_data["PubmedArticle"]
        ...
        try:
            ...
            if article_is_relevant:
                summary = self.summarize_study(
                    article_text=abstract, question=question, model="gemini-1.5-flash"
                )
            ...
            if "PMID" in article["MedlineCitation"].keys():
                print(
                    f"Could not find {err} for article with PMID = {article['MedlineCitation']['PMID']}"
                )
            else:
